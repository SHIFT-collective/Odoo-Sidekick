...
IMPORTANT — what the mode gate is and is not:
    ...
    user and prove it with `python -m scripts.verify_profile <profile>`.
...
Usage as a CLI:
    ...
    python -m scripts.odoo_client <profile> res.partner write \\
...
def _hint_for(status: int, name: str, message: str, model: str, method: str) -> str:
    ...
    if status == 404 and "does not exist" in msg and "model" in msg.lower():
        ...
        return (f"The model name is wrong. Fuzzy-search installed models with: "
                f"python -m scripts.introspect <profile> --find {stem or '<word>'}")
    ...
    if m:
        return (f"Unknown field {m.group(1)!r} on {model or 'this model'}. List real fields with: "
                f"python -m scripts.introspect <profile> --model {model or '<model>'} — "
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    surgery (sed etc.) — use `python -m scripts.profiles set ...`, which
    ...
    try:
        ...
        raise ProfileError(
            ...
            "Install with `pip install pyyaml`, or convert the file to JSON with "
            "`python -m scripts.profiles migrate`."
        ) from e
    ...
    def load(cls, name: str | None = None, profiles_path: Path | None = None) -> "Profile":
        ...
        if not path.exists():
            raise ProfileError(
                ...
                "`python -m scripts.profiles add <name> --url ... --api-key ...`."
            )
...
def _cli(argv: list[str] | None = None) -> int:
    ...
    if risk and not args.allow_inline_binary:
        print(f"REFUSED: {risk}.\n"
              f"Use `python -m scripts.get_attachment {args.profile} --id <id> --out <file>` "
